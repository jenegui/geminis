...
class JFormer {
	...
	function addJFormComponent($jFormComponent) {
		...
		if (empty ( $this->jFormPageArray )) {
			$this->addJFormPage ( new JFormPage ( $this->id . '_page1', array (
					'anonymous' => true 
			) ) );
		}
		...
		if (! empty ( $lastJFormSection ) && $lastJFormSection->anonymous) {
			...
			$anonymousSection = new JFormSection ( $currentJFormPage->id . '_section' . (sizeof ( $currentJFormPage->jFormSectionArray ) + 1), array (
					'anonymous' => true 
			) );
		...
		if ($formState == 'newForm') {
			...
			if ($response ['status'] == 'failure') {
				...
				$response ['response'] = array (
						'failureNoticeHtml' => 'Form already exists. Either choose to resume the form, or enter a different password to create a new form.' 
				);
			}
	...
	public function createSaveState($formData) {
		...
		if ($query->errno) {
			...
			$response = array (
					'status' => 'success',
					"response" => 'Successfully created a new form state.' 
			);
		}
	...
	public function saveState($formData) {
		...
		if ($query->errno) {
			...
			$response = array (
					'status' => 'success',
					"response" => 'Successfully updated the form state.' 
			);
		}
	...
	public function getSavedState() {
		...
		if ($query->errno) {
			...
			$response = array (
					"status" => "failure",
					"response" => array (
							'failureNoticeHtml' => 'No form exists for that username and password combination. Try again or start a new form.' 
					) 
			);
		} else {
			$resultArray = array ();
	...
	public function createSaveStateTable() {
		...
		if ($query->errno) {
			...
			$response = array (
					"status" => "success",
					"response" => 'Table `' . $this->saveState ['database'] ['table'] . '` created successfully.' 
			);
		}
	...
	function processRequest($silent = false) {
		...
		if (isset ( $_SERVER ['CONTENT_LENGTH'] ) && empty ( $_POST )) {
			$this->setStatus ( 'success', array (
					'failureNoticeHtml' => 'Your request (' . round ( $_SERVER ['CONTENT_LENGTH'] / 1024 / 1024, 1 ) . 'M) was too large for the server to handle. ' . ini_get ( 'post_max_size' ) . ' is the maximum request size.' 
			) );
			echo '
		...
		if (isset ( $_POST ['jFormer'] ) && $this->id == $_POST ['jFormerId'] || isset ( $_POST ['jFormerTask'] )) {
			...
			if (isset ( $_POST ['jFormer'] )) {
				...
				if (! $this->validationPassed) {
					$this->setStatus ( 'failure', array (
							'validationFailed' => $this->validationResponse 
					) );
				} else {
					try {
					...
					if (empty ( $onSubmitErrorMessageArray )) {
						...
						$this->setStatus ( 'failure', array (
								'failureHtml' => $onSubmitErrorMessageArray 
						) );
					}
				}
	...
	function getHtml() {
		...
		if ($this->splashPageEnabled || $this->saveStateEnabled) {
			...
			if ($this->saveStateEnabled) {
				...
				$saveStateStatus = new jFormComponentMultipleChoice ( 'saveStateStatus', 'Starting a new form?', array (
						array (
								'value' => 'newForm',
								'label' => 'Yes, let me start a new form',
								'checked' => true 
						),
						array (
								'value' => 'resumeForm',
								'label' => 'No, I want to continue a previous form' 
						) 
				), array (
				...
				$this->saveState ['components'] = array (
						$saveStateIdentifier->id => $saveStateIdentifier->getOptions (),
						$saveStateStatus->id => $saveStateStatus->getOptions (),
						$saveStatePassword->id => $saveStatePassword->getOptions () 
				);
			...
			if (! isset ( $this->splashPage ['customButtonId'] )) {
				$splashLi = new JFormElement ( 'li', array (
						'class' => 'splashLi' 
				) );
				$splashButton = new JFormElement ( 'button', array (
						'class' => 'splashButton' 
				) );
		...
		$previousButtonLi = new JFormElement ( 'li', array (
				'class' => 'previousLi',
				'style' => 'display: none;' 
		) );
		$previousButton = new JFormElement ( 'button', array (
		...
		$nextButtonLi = new JFormElement ( 'li', array (
				'class' => 'nextLi' 
		) );
		$nextButton = new JFormElement ( 'button', array (
				'class' => 'nextButton' 
		) );
		...
		$jFormPageWrapper = new JFormElement ( 'div', array (
				'class' => 'jFormPageWrapper' 
		) );
		$jFormPageScroller = new JFormElement ( 'div', array (
				'class' => 'jFormPageScroller' 
		) );
		...
		$pageWrapperContainer = new JFormElement ( 'div', array (
				'class' => 'jFormWrapperContainer' 
		) );
		...
		if (! empty ( $this->afterControl )) {
			$subSubmitInstructions = new JFormElement ( 'div', array (
					'class' => 'jFormerAfterControl' 
			) );
...
class JFormPage {
	...
	function addJFormComponent($jFormComponent) {
		...
		if (empty ( $this->jFormSectionArray )) {
			$this->addJFormSection ( new JFormSection ( $this->id . '_section1', array (
					'anonymous' => true 
			) ) );
		}
		...
		if (! empty ( $lastJFormSection ) && $lastJFormSection->anonymous) {
			...
			// Create an anonymous section
			$anonymousSection = new JFormSection ( $this->id . '_section' . (sizeof ( $this->jFormSectionArray ) + 1), array (
					'anonymous' => true 
			) );
	...
	function getOptions() {
		...
		if (! empty ( $this->dependencyOptions )) {
			...
			if (isset ( $this->dependencyOptions ['dependentOn'] ) && ! is_array ( $this->dependencyOptions ['dependentOn'] )) {
				$this->dependencyOptions ['dependentOn'] = array (
						$this->dependencyOptions ['dependentOn'] 
				);
			}
						...
						if ($validationResponse != 'success') {
							...
							if (is_array ( $validationResponse )) {
								...
								if (is_string ( $validationResponse )) {
									$this->errorMessageArray [$instanceKey] = array_merge ( $this->errorMessageArray [$instanceKey], array (
											$validationResponse 
									) );
								} else {
									$this->errorMessageArray [$instanceKey] = array_merge ( $this->errorMessageArray [$instanceKey], array (
											'There was a problem validating this component on the server.' 
									) );
								}
							}
				...
				foreach ( $this->validationOptions as $validationType => $validationOptions ) {
					...
					if ($validationResponse != 'success') {
						...
						if (is_array ( $validationResponse )) {
							...
							if (is_string ( $validationResponse )) {
								$this->errorMessageArray = array_merge ( array (
										$validationResponse 
...
class JFormComponentAddress extends JFormComponent {
	...
	function getOption($optionValue, $optionLabel, $optionSelected, $optionDisabled) {
		...
		$option = new JFormElement ( 'option', array (
				'value' => $optionValue 
		) );
	...
	public function required($options) {
		...
		if ($options ['value']->addressLine1 == '') {
			array_push ( $errorMessageArray, array (
	...
	public function required($options) {
		...
		if (empty ( $options ['value']->cardNumber )) {
			...
			if (preg_match ( '/[^\d]/', $options ['value']->cardNumber )) {
				array_push ( $errorMessageArray, array (
						'Card number may only contain numbers.' 
				) );
			}
			if (strlen ( $options ['value']->cardNumber ) > 16 || strlen ( $options ['value']->cardNumber ) < 13) {
				array_push ( $errorMessageArray, array (
						'Card number must contain 13 to 16 digits.' 
				) );
			}
		}
		...
		if ($this->showSecurityCode && empty ( $options ['value']->securityCode )) {
			...
			if (preg_match ( '/[^\d]/', $options ['value']->securityCode )) {
				array_push ( $errorMessageArray, array (
						'Security code may only contain numbers.' 
				) );
			}
...
class JFormComponentDropDown extends JFormComponent {
	...
	public static function getCountryArray($selectedCountry = null) {
		...
		$countryArray = array (
				...
				array (
						'value' => 'CI',
						'label' => 'Cote D\'Ivorie (Ivory Coast)' 
				),
				array (
						'value' => 'HR',
				...
				array (
						'value' => 'CD',
						'label' => 'Democratic Republic of Congo (Zaire)' 
				),
				array (
						'value' => 'DK',
						'label' => 'Denmark' 
				...
				array (
						'value' => 'FX',
						'label' => 'France), Metropolitanarray(' 
				),
				array (
						'value' => 'GF',
						'label' => 'French Guinea' 
	...
	function __toString() {
		...
		foreach ( $optGroupArray as $optGroup ) {
			${$optGroup} = new JFormElement ( 'optgroup', array (
					'label' => $optGroup 
			) );
		}
...
class JFormComponentFile extends JFormComponent {
	...
	public function extension($options) {
		...
		$messageArray = array (
				'Must have the .' . $options->extension . ' extension.' 
		);
	...
	public function extensionType($options) {
		...
		if (is_array ( $options ['extensionType'] )) {
			...
			$messageArray = array (
					'Must be an ' . $options ['extensionType'] . ' file type.' 
			);
		}
	...
	public function size($options) {
		...
		return $fileSizeInKb <= $options ['size'] ? 'success' : array (
				'File must be smaller then ' . $options ['size'] . 'kb. File is ' . round ( $fileSizeInKb, 2 ) . 'kb.' 
		);
	...
	public function imageDimensions($options) {
		...
		if (! $imageInfo) {
			...
			if ($width > $options ['width']) {
				$errorMessageArray [] = array (
						'The image must be less then ' . $options ['width'] . 'px wide. File is ' . $width . 'px.' 
				);
			}
			if ($height > $options ['height']) {
				$errorMessageArray [] = array (
						'The image must be less then ' . $options ['height'] . 'px tall. File is ' . $height . 'px.' 
				);
			}
		}
	...
	public function minImageDimensions($options) {
		...
		if (! $imageInfo) {
			...
			if ($width < $options ['width']) {
				$errorMessageArray [] = array (
						'The image must at least then ' . $options ['width'] . 'px wide. File is ' . $width . 'px.' 
				);
			}
			if ($height < $options ['height']) {
				$errorMessageArray [] = array (
						'The image must at least then ' . $options ['height'] . 'px tall. File is ' . $height . 'px.' 
				);
			}
		}
		return empty ( $errorMessageArray ) ? 'success' : $errorMessageArray;
...
class JFormComponentLikert extends JFormComponent {
	...
	function __toString() {
		...
		// Create the table
		$table = new JFormElement ( 'table', array (
				'class' => 'jFormComponentLikertTable' 
		) );
		
		// Generate the first row
		if ($this->showTableHeading) {
			$tableHeadingRow = new JFormElement ( 'tr', array (
					'class' => 'jFormComponentLikertTableHeading' 
			) );
		...
		foreach ( $this->statementArray as $statement ) {
			...
			// Set the statement
			$statementRow = new JFormElement ( 'tr', array (
					'class' => $statementRowClass 
			) );
			$statementColumn = new JFormElement ( 'td', array (
					'class' => 'jFormComponentLikertStatementColumn' 
			) );
...
class JFormComponentMultipleChoice extends JFormComponent {
	...
	public function minOptions($options) {
		...
		$errorMessageArray = array (
				'You must select more than ' . $options ['minOptions'] . ' options' 
		);
	...
	public function maxOptions($options) {
		...
		$errorMessageArray = array (
				'You may select up to ' . $options ['maxOptions'] . ' options. You have selected ' . sizeof ( $options ['value'] ) . '.' 
		);
...
class JFormComponentSingleLineText extends JFormComponent {
	...
	public function length($options) {
		...
		$messageArray = array (
				'Must be exactly ' . $options ['length'] . ' characters long. Current value is ' . strlen ( $options ['value'] ) . ' characters.' 
		);
	...
	public function maxLength($options) {
		...
		$messageArray = array (
				'Must be less than ' . $options ['maxLength'] . ' characters long. Current value is ' . strlen ( $options ['value'] ) . ' characters.' 
		);
	...
	public function maxFloat($options) {
		...
		$messageArray = array (
				'Must be numeric and cannot have more than ' . $options ['maxFloat'] . ' decimal place(s).' 
		);
	...
	public function maxValue($options) {
		...
		$messageArray = array (
				'Must be numeric with a maximum value of ' . $options ['maxValue'] . '.' 
		);
	...
	public function minLength($options) {
		...
		$messageArray = array (
				'Must be at least ' . $options ['minLength'] . ' characters long. Current value is ' . strlen ( $options ['value'] ) . ' characters.' 
		);
	...
	public function minValue($options) {
		...
		$messageArray = array (
				'Must be numeric with a minimum value of ' . $options ['minValue'] . '.' 
		);
